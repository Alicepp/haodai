...
function sendLog($message, $level = 'info') {
  defined('LOG_LOCAL') && with(qb_Logs::getInstance())->sendTelnet(LOG_LOCAL, $message, $level);

  defined('LOG_SYS') && with(qb_Logs::getInstance())->sendTcp(LOG_SYS, $message, $level);
}
...
function getCssModule($uri = '') {
  ...
  $method === $uri[count($uri) - 1] && array_pop($uri);
  count($uri) > 1 && $class === $uri[count($uri) - 1] && array_pop($uri);
